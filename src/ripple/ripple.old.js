...
var shaderProgramMain,
  shaderProgramFixed,
  shaderProgramAcoustic,
  shaderProgramDraw,
  shaderProgramMode;
...
function initShader(fs, vs, prefix) {
  ...
  shaderProgram.dampingAttribute = gl.getAttribLocation(
    shaderProgram,
    "aDamping"
  );
...
function initShaders() {
  ...
  shaderProgramMain.brightnessUniform = gl.getUniformLocation(
    shaderProgramMain,
    "brightness"
  );
  shaderProgramMain.colorsUniform = gl.getUniformLocation(
    shaderProgramMain,
    "colors"
  );

  shaderProgram3D = initShader("shader-draw-fs", "shader-3d-vs", null);
  shaderProgram3D.brightnessUniform = gl.getUniformLocation(
    shaderProgram3D,
    "brightness"
  );
  shaderProgram3D.colorsUniform = gl.getUniformLocation(
    shaderProgram3D,
    "colors"
  );
  shaderProgram3D.xOffsetUniform = gl.getUniformLocation(
    shaderProgram3D,
    "xOffset"
  );

  shaderProgramFixed = initShader("shader-simulate-fs", "shader-vs", null);
  shaderProgramFixed.stepSizeXUniform = gl.getUniformLocation(
    shaderProgramFixed,
    "stepSizeX"
  );
  shaderProgramFixed.stepSizeYUniform = gl.getUniformLocation(
    shaderProgramFixed,
    "stepSizeY"
  );

  shaderProgramAcoustic = initShader(
    "shader-simulate-fs",
    "shader-vs",
    "#define ACOUSTIC 1\n"
  );
  shaderProgramAcoustic.stepSizeXUniform = gl.getUniformLocation(
    shaderProgramAcoustic,
    "stepSizeX"
  );
  shaderProgramAcoustic.stepSizeYUniform = gl.getUniformLocation(
    shaderProgramAcoustic,
    "stepSizeY"
  );
...
function initBuffers() {
  ...
  if (!sourceBuffer) sourceBuffer = gl.createBuffer();
  ...
  if (!screen3DTextureBuffer) screen3DTextureBuffer = gl.createBuffer();
  ...
  setPosRect(
    windowOffsetX,
    windowOffsetY,
    gridSizeX - windowOffsetX,
    gridSizeY - windowOffsetY
  );
  ...
  if (!simVertexPositionBuffer) simVertexPositionBuffer = gl.createBuffer();
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(simTextureCoord),
    gl.STATIC_DRAW
  );
  simVertexTextureCoordBuffer.itemSize = 2;
  simVertexTextureCoordBuffer.numItems = simPosition.length / 2;

  if (!simVertexDampingBuffer) simVertexDampingBuffer = gl.createBuffer();
...
function simulate() {
  ...
  gl.vertexAttribPointer(
    ...
    simVertexPositionBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.vertexAttribPointer(
    ...
    simVertexTextureCoordBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.vertexAttribPointer(
    ...
    simVertexDampingBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
    0
...
function drawHandle(x, y) {
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, sourceBuffer);
  var cx = -1 + (2 * (x + 0.5)) / windowWidth;
  var cy = +1 - (2 * (y + 0.5)) / windowHeight;
  var ox = 0.01;
  var oy = 0.01;
  var coords = [
    cx - ox,
...
function drawLineSource(x, y, x2, y2, f, gauss) {
  ...
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(srcCoords), gl.STATIC_DRAW);
  gl.vertexAttribPointer(
    shaderProgramDrawLine.vertexPositionAttribute,
    sourceBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
  ...
  gl.vertexAttribPointer(
    shaderProgramDrawLine.colorAttribute,
...
function loadMatrix(mtx) {
  ...
  if (sim.drawingSelection > 0) {
    ...
    mtx[0] = +2 / gridSizeX;
    mtx[5] = -2 / gridSizeY;
    mtx[12] = -1 + (0.5 + windowOffsetX) * mtx[0];
    mtx[13] = +1 + (0.5 + windowOffsetY) * mtx[5];
  }
  mat4.multiply(
    mtx,
    [
...
function setupForDrawing(v) {
  if (sim.drawingSelection > 0) {
    gl.vertexAttrib4f(
      shaderProgramDraw.colorAttribute,
      sim.drawingColor[0] * sim.drawingSelection,
      sim.drawingColor[1] * sim.drawingSelection,
      sim.drawingColor[2] * sim.drawingSelection,
      sim.drawingColor[3]
    );
  } else {
    var rttFramebuffer = renderTexture1.framebuffer;
...
function drawScene(bright) {
  ...
  gl.vertexAttribPointer(
    ...
    laptopScreenVertexPositionBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.vertexAttribPointer(
    ...
    laptopScreenVertexTextureCoordBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
    0
...
function drawScene3D(bright) {
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, screen3DTextureBuffer);
  gl.vertexAttribPointer(
    shaderProgram3D.textureCoordAttribute,
    screen3DTextureBuffer.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.uniform1f(shaderProgram3D.brightnessUniform, bright * 0.1);
...
document.passCanvas = function passCanvas(cv, sim_) {
  ...
  sim.acoustic = false;
  sim.updateRipple = function updateRipple(bright) {
    drawScene(bright);
  };
  sim.updateRipple3D = function updateRipple3D(bright) {
    drawScene3D(bright);
  };
  ...
  sim.drawSource = function(x, y, f) {
    drawSource(x, y, f);
  };
  sim.drawLineSource = function(x, y, x2, y2, f, g) {
    drawLineSource(x, y, x2, y2, f, g);
  };
  sim.drawPhasedArray = function(x, y, x2, y2, f1, f2) {
    drawPhasedArray(x, y, x2, y2, f1, f2);
  };
